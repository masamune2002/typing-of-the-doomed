extends Enemy
class_name ShotgunGuy

# DOOM sprite naming: SPOSxY where x=frame letter, Y=angle (1-8, 0=no rotation)
# A-B: Idle, C-D: Walk, E-F: Attack, G: Pain, H-L: Death (L=corpse)
const SPRITE_PREFIX = "SPOS"
...
func _updateSprite() -> void:
	...
	if _sprites.has(spriteName):
		...
		print("ShotgunGuy: sprite '", spriteName, "' not found. Available: ", _sprites.keys())
