extends Enemy
class_name Cyberdemon

const SPRITE_PREFIX = "CYBR"
const IDLE_FRAMES = ["A", "B"]
const ATTACK_FRAMES = ["E", "F"]
const PAIN_FRAMES = ["G"]
const DEATH_FRAMES = ["H", "I", "J", "K", "L", "M", "N", "O", "P"]
const ANGLE_ZERO_FRAMES = ["G", "H", "I", "J", "K", "L", "M", "N", "O", "P"]
...
const PROJECTILE_SPRITE_PREFIX = "MISL"
...
func _updateSprite() -> void:
	...
	if _sprites.has(spriteName):
		...
		print("Cyberdemon: sprite '", spriteName, "' not found. Available: ", _sprites.keys())
...
func _startSpriteDeath() -> void:
	...
	_deathComplete = true
	var corpseKey = SPRITE_PREFIX + "P0"
...
func _attack(target: Player) -> void:
	...
	if stateMachine.currentStateKey == Enums.ENEMY_STATE.ATTACKING:
		...
		projectile.sprite_prefix = PROJECTILE_SPRITE_PREFIX
