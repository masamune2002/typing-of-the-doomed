...
func _ready() -> void:
	...
	debugLabel.position = barrelLabel.position + Vector3(0, -0.3, 0)
...
func _applyDoomFont() -> void:
	...
	if doomFont != null:
		for label in [barrelLabel, typedLabel, debugLabel]:
			if label != null:
				label.font = doomFont
				label.font_size = 16
...
func _setFullWordLabel() -> void:
	...
	barrelLabel.text = fullWord.to_upper()
...
func _updateTypedLabel() -> void:
	...
	barrelLabel.text = remaining.to_upper()
